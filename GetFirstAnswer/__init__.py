...
import json
from shared_code.config import get_openai_api_key, get_openai_endpoint
...
"""
        ...
        try:
            ...
            return func.HttpResponse(
                json.dumps({"error": "OpenAI API request timed out"}),
                status_code=502,
                mimetype="application/json"
            )
            ...
            return func.HttpResponse(
                json.dumps({"error": "Failed to connect to OpenAI API"}),
                status_code=502,
                mimetype="application/json"
            )
        ...
        if response.status_code != 200:
            ...
            return func.HttpResponse(
                json.dumps({"error": f"OpenAI API returned status {response.status_code}"}),
                status_code=502,
                mimetype="application/json"
            )
        ...
        try:
            ...
            logging.error(f"Invalid JSON response from OpenAI API: {response.text[:500]}")
            return func.HttpResponse(
                json.dumps({"error": "Invalid response from OpenAI API"}),
                status_code=502,
                mimetype="application/json"
            )
        ...
        try:
            ...
            logging.error(f"Malformed OpenAI response structure: {e} - {response_json}")
            return func.HttpResponse(
                json.dumps({"error": "No response generated by model"}),
                status_code=502,
                mimetype="application/json"
            )
